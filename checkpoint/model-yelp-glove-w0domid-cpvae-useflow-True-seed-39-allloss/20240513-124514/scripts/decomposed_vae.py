...
class DecomposedVAE:
    def __init__(self, train, valid, test, feat,pos_list,neg_list, bsz, save_path, logging, log_interval, num_epochs,
                 enc_lr, dec_lr, warm_up, kl_start, beta1, beta2, srec_weight, reg_weight,ic_weight,
                 aggressive, text_only,n_domains,train_schema,wdomid, vae_params):
    ...
    def train(self, epoch):
        ...
        for idx in np.random.permutation(range(self.nbatch)):
            ...
            vae_logits, vae_kl1_loss, vae_kl2_loss, sparse_loss,cmask_loss = self.vae.loss(
                x=batch_data, feat=batch_feat,u=batch_domid, no_ic=0)
