...
def main(args):
    ...
    train_data = MonoTextData(train_pth, n_domains=args.n_domains,vocab_size=10000)
    ...
    dev_data = MonoTextData(dev_data_pth, n_domains=args.n_domains, vocab=vocab)
    ...
    test_data = MonoTextData(test_data_pth, n_domains=args.n_domains, vocab=vocab)
    ...
    params["vae_params"]["n_domains"] = args.n_domains
    ...
    train_data, train_feat, train_domid = train_data.create_data_batch_feats(32, train_feat, device)
    ...
    with open(os.path.join(args.load_path, 'generated_results.txt'), "w") as f:
        ...
        while idx < n_samples:
            ...
            var_id = neg_idx if label else pos_idx #if original sent is pos/1, target sent should be neg/0
            ...
            domid = torch.tensor(test_domid[idx:_idx], dtype=torch.int, requires_grad=False, device=device)
            z1, _ = model.vae.lstm_encoder(text[:min(text.shape[0], 10)],domid)
            ori_z2, _ = model.vae.mlp_encoder(feat,domid)
